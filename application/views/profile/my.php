...
<head>
  ...
  <title>个人资料卡 / <?php echo $this->Setting_model->get('systemName'); ?></title>
</head>
...
<div class="panel panel-default">
	<div class="panel-body">
		...
		<div class="form-group">
			...
			<select class="form-control" id="sex" onchange='$("#idCard").focus();'>
		...
		<div class="form-group">
			...
			<input type="number" class="form-control" id="phone" onkeyup='if(event.keyCode==13)$("#description").focus();' value="<?=$info['phone'];?>">
		</div>
		<br>
		<div class="form-group">
		...
		<p class="help-block" style="color:red;font-weight:bold;font-size:16px;">请先点击右侧上传，再确认修改资料！<button type="button" class="btn btn-primary" onclick='$("#photoModal").modal("show");'>查看原照片</button></p>
		</form>
		...
		<button class="btn btn-success btn-block" onclick='updateProfile()'>确 认 修 改 资 料 &gt;</button>
	</div>
</div>
...
function updateProfile(){
	...
	$.ajax({
		url:"<?=base_url('profile/toEditMy');?>",
		type:"post",
		data:{<?php echo $this->ajax->showAjaxToken(); ?>,'name':name,"sex":sex,"idCard":idCard,"phone":phone,"enrollYear":enrollYear,"classNum":classNum,"description":description,"photoUrl":photoUrl},
		dataType:'json',
		error:function(e){
...
function uploadPhoto(){
	...
	if($("#myfile").val().length>0){
		...
		$.ajax({
			url:'<?=base_url('profile/toUploadPhoto');?>',
			type: "post",
			data: formData,
			dataType:"json",
			cache: false,
